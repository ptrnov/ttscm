...
import com.cudocomm.troubleticket.adapter.TopTenSuspectAdapter;
...
public class TTSuspectFragment extends BaseFragment {

    private View rootView;
    private RecyclerView topTenActiveRV;

    private TopTenSuspectAdapter topTenActiveAdapter;
    private List<TopTenSuspect> topTenActives = new ArrayList<>();
...
//    private static final int REQUEST_CODE = 200;
    ...
    public static TTSuspectFragment newInstance(List<TopTenSuspect> topTenActives) {
        TTSuspectFragment fragment = new TTSuspectFragment();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            topTenActives = (List<TopTenSuspect>) getArguments().getSerializable("list");
        }
    }

    public TTSuspectFragment() {
    ...
    private void updateComponent() {
                ...
                if(topTenActiveAdapter == null) {
                    topTenActiveAdapter = new TopTenSuspectAdapter(topTenActives);
                } else {
        ...
        for(int i=0; i<topTenActives.size(); i++) {
            if(!headers.contains(topTenActives.get(i).getSuspectName()))
                headers.add(topTenActives.get(i).getSuspectName());
