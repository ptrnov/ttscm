...
public class TTLongestFragment extends BaseFragment {
...
//    private static final int REQUEST_CODE = 200;
    ...
    public static TTLongestFragment newInstance(List<Ticket> topTenActives) {
        TTLongestFragment fragment = new TTLongestFragment();
    ...
    private void updateComponent() {
        ...
        topTenActiveRV.setAdapter(ticketAdapter);

        for(int i=0; i<tickets.size(); i++) {
            if(!headers.contains(tickets.get(i).getTicketNo()))
                headers.add(tickets.get(i).getTicketNo());
            float aging = CommonsUtil.countAgingInDay(tickets.get(i).getTicketDate());
            entries.add(new BarEntry(i, aging));
        }
